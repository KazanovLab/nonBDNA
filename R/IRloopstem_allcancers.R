...
dataBDNA[, cancerType := tstrsplit(Cancer,'-')[1]] 
...
for(cancer in c(#"BLCA-US",
  ...
  for(s in samples){
    ...
    results <- rbind(results, data.table("cancerType"=strsplit(cancer,'-')[[1]][[1]], "cancer"=cancer, "sample"=s, "type"="1LoopC", "density"=log(loopCdensity/DBNAdensity)))
    results <- rbind(results, data.table("cancerType"=strsplit(cancer,'-')[[1]][[1]], "cancer"=cancer, "sample"=s, "type"="2loopAPB", "density"=log(loopAPBdensity/DBNAdensity)))
    results <- rbind(results, data.table("cancerType"=strsplit(cancer,'-')[[1]][[1]], "cancer"=cancer, "sample"=s, "type"="3stemAPB", "density"=log(stemAPBdensity/DBNAdensity)))
...
for(p in c(1,2,3)){
  ...
  if(p == 1){
    cvec <- c("BOCA","BTCA","CLLE","CMDI","COAD","DLBC","EOPC","ESAD","GACA","GBM","KICH","KIRC")
    wdth <- 300
  }
  else if(p == 2){
    cvec <- c("KIRP","LAML","LGG","LICA","LIHC","LINC","LIRI","MALY","MELA","ORCA","OV","PACA")
    wdth <- 300
  }
  else if(p == 3){
    cvec <- c("PAEN","PBCA","PRAD","READ","RECA","SARC","SKCM","STAD","THCA","UCEC")
    wdth <- 250
  }
    
  resTmp <- results[cancerType %in% cvec] 
  
  ggplot(resTmp, aes(x=cancerType,y=density,fill=type,color=type)) +
  ...
  ggsave(paste0("/Users/mar/BIO/PROJECTS/APOBEC/NONBDNA/IRloopstem_allcancers/0_pics/ir_allcancers_new_",p,".tiff"),units="mm",dpi=300,width=wdth,height=80)
  
}  
  
  # Significances
  
  resultsSignif <- data.table()
  
  for(ctype in unique(results$cancerType)){
    dt <- results[cancerType == ctype]
    dt <- dt[is.finite(density)]
    
    dtSignif1 <- dt[type %in% c("1LoopC","2loopAPB")]
    res1 <- wilcox.test(density ~ type, dtSignif1)
    
    dtSignif2 <- dt[type %in% c("1LoopC","3stemAPB")]
    res2 <- wilcox.test(density ~ type, dtSignif2)
    
    dtSignif3 <- dt[type %in% c("2loopAPB","3stemAPB")]
    res3 <- wilcox.test(density ~ type, dtSignif3)
    
    resultsSignif <- rbind(resultsSignif, data.table("cancerType"=ctype,
                                                     "1LoopC_2loopAPB"=res1$p.value,
                                                     "1LoopC_3stemAPB"=res2$p.value,
                                                     "2loopAPB_3stemAPB"=res3$p.value))
  }
  
  
  write.csv(resultsSignif,"/Users/mar/BIO/PROJECTS/APOBEC/NONBDNA/IRloopstem_allcancers/0_pics/signifTable.txt",row.names = F,quote = F)
